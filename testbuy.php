...
if (isset($_POST['buy'])) {
	$e_name = htmlspecialchars($_POST['nameEvent']);
	$img_src = htmlspecialchars($_POST['image']);
}
...
<input type='submit' name='buttonBuy' value='{$buy}'/>
...
while ($i < sizeof($free)) {
	...
	$ceel = "<td class = '{$sort}'><input type='checkbox'  class='chk' name='checkbox[]' value='{$free[$i]}'>{$p_teg}</td>";
	$noceel = "<td class = '{$sort}'></br><input type='hidden'  value='{$free[$i]}' > {$p_teg}</td>";
	$paid = "<td class = '{$sort}'></br><input type='hidden'  value='{$free[$i]}' > {$p_teg}</td>";
	if ($a) {	
